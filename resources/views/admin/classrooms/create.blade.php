        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-150 overflow-hidden">
                <div class="p-6 bg-gradient-to-r from-gray-50 to-indigo-50/20 border-b border-gray-150">
                    ...
                    <p class="text-xs text-gray-500">{{ __('Provide details to register this room in the availability search registry.') }}</p>
                </div>

                <form method="POST" action="{{ route('admin.classrooms.store') }}" class="p-6 space-y-6">
                    @csrf

                    <!-- Room Name -->
                    <div class="space-y-1">
                        <label for="name" class="block text-sm font-bold text-gray-700">{{ __('Room Name / Number') }}</label>
                        <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="{{ __('e.g. Room 304, Lecture Hall A') }}" required
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location') }}" placeholder="{{ __('e.g. North Building, Science Block') }}" required
                            class="w-full rounded-lg border-gray-250 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 shadow-sm transition">
                        @error('location')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="number" name="capacity" id="capacity" value="{{ old('capacity') }}" min="1" placeholder="{{ __('e.g. 30') }}" required
                                class="w-full rounded-lg border-gray-250 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 shadow-sm transition">
                            @error('capacity')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="equipment" id="equipment" value="{{ old('equipment') }}" placeholder="{{ __('e.g. projector, whiteboard, smartboard, computers') }}"
                            class="w-full rounded-lg border-gray-250 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 shadow-sm transition">
